/* Map app — persistent session, accurate geolocation, live pins.
   City Harvest:
     - No side tabs. Actions appear inline in History only:
       * Needer sees “Contact City Harvest Delivery” next to Mark Received (after helper confirms).
       * Helper sees “Handed to City Harvest” next to Mark Given.
     - Prints include courier & feedback.
   Admin:
     - Posted Resources & Pickup Requests stay live and printable.
   UI:
     - Removed any leftover “Contact City Harvest Delivery” / “Given to City Harvest” tabs.
*/
...
document.getElementById('loginForm')?.addEventListener('submit',e=>{
  ...
  if(role==='admin'){
    ...
    const pw=adminPasswordInput?.value; if(!pw) return alert('Please enter the admin password.'); if(pw!==getAdminPassword()) return alert('Invalid admin password.');
  }
